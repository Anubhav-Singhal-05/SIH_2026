...
export default function InheritanceActivation() {
  ...
  const sigCount = effectiveRule.authoritySet.filter((a) => a.signed).length
  const canActivate = sigCount >= effectiveRule.signaturesRequired && effectiveRule.status !== 'ACTIVE' && effectiveRule.status !== 'CLOSED' && effectiveRule.status !== 'NOT_CONFIGURED'

  // batch execution once ACTIVE
  useEffect(() => {
    if (effectiveRule.status === 'ACTIVE' && effectiveRule.batchProgress.length === 0) {
      const owned = assets.filter((a) => a.ownerDid === effectiveRule.ownerDid && a.status === 'CONFIRMED')
      patchInheritance({ batchProgress: owned.map((a) => ({ assetId: a.assetId, name: a.name, state: 'PENDING' })) })
      owned.forEach((a, i) => {
  ...
  }, [effectiveRule.status, effectiveRule.ownerDid])
  useEffect(() => () => timers.current.forEach(clearTimeout), [])

  const sign = (label) => patchInheritance({ authoritySet: effectiveRule.authoritySet.map((a) => (a.label === label ? { ...a, signed: true } : a)) })
  const activate = () => {
    patchInheritance({ status: 'ACTIVATING' })
    const id = startOperation({ type: 'inheritance_activate', label: 'activate inheritance', relatedId: effectiveRule.ownerDid })
    setOpId(id)
    ...
    <div>
      ...
      <div className='grid grid-cols-5 gap-3 max-w-[1200px]'>
        <div className='col-span-3 space-y-3'>
          <Panel title='Authority signature collection' actions={<Badge status={effectiveRule.status} />}>
            <div className='space-y-2'>
              {effectiveRule.authoritySet.map((a) => (
                <div key={a.label} className='flex items-center justify-between inset-panel px-3 py-2'>
                  ...
                  {a.signed ? <Badge status='CONFIRMED' /> : <Btn className='!py-1' onClick={() => sign(a.label)} disabled={effectiveRule.status === 'ACTIVE'}><PenLine size={11} /> Simulate authority signing</Btn>}
                </div>
              ))}
            </div>
            <p className='font-mono text-[11px] text-steel-400 mt-3'>&gt; {sigCount} of {effectiveRule.signaturesRequired} required signatures collected</p>
            {sigCount >= effectiveRule.signaturesRequired && effectiveRule.status !== 'ACTIVE' && <p className='font-mono text-[11px] text-ok mt-1'>&gt; quorum reached / activation available</p>}
          </Panel>
          <Panel title='Batch execution per asset' pad={false}>
            {effectiveRule.status !== 'ACTIVE' ? (
              ...
              <table className='w-full text-[12px]'>
                ...
                <tbody>
                  {effectiveRule.batchProgress.map((b) => (
        ...
        <div className='col-span-2 space-y-3'>
          <Panel title='Activation'>
            <KeyRow k='Rule status'><Badge status={effectiveRule.status} /></KeyRow>
            <KeyRow k='Nominee'><span className='font-mono text-[12px]'>{effectiveRule.defaultNomineeDid || '— none configured —'}</span></KeyRow>
            <KeyRow k='Overrides'><span className='font-mono text-[12px]'>{effectiveRule.perAssetOverrides.length}</span></KeyRow>
            <Btn variant='primary' className='w-full justify-center mt-3' disabled={!canActivate} onClick={activate}>
              {effectiveRule.status === 'ACTIVE' ? 'Inheritance active' : canActivate ? 'Activate via step-up' : 'Quorum not reached'}
            </Btn>
